...
use std::convert::{TryFrom, TryInto}; //TryFrom特质用于将一中类型转为另一种类型
...
//为Color结构体实现TryFrom特质，实现了TryFrom trait的结构体可以通过Structor::try_from()进行类型转换
...
fn main() {
    //实现了TryFrom trait的结构体可以通过::try_from()进行类型转换
